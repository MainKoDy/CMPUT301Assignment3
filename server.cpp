    ...
    if (((stoi(port)-64000) * (stoi(port)-5000)) > 0) {
        ...
        while(true) {  
            ...
            if (activity = select( max_sd + 1 , &readfds , NULL , NULL , &timeout) <= 0) {
                printf("Timeout occured. Exiting server program ... \n");

                printf("\n");
                printf("SUMMARY \n");
                // double totalTime = (chrono::duration_cast<chrono::nanoseconds>(end - start).count())/1000000; // return current execution time
                // double totalTimeElapsed = totalTime / 1000;
                if (hasConnected == true) {
                    sprintf(totalTimeBuf, "%.2f", sumTime); 
                ...
                if (FD_ISSET( sd , &readfds)) {  
                    ...
                    //Echo back the message that came in 
                        ...
                        // Host given
                            ...
                            double ttimeNow = (chrono::duration_cast<chrono::nanoseconds>(end - start).count())/1000000; // return current execution time
